...
#     ground_truth = pd.read_csv(test_img_path.replace('.jpg', '.txt'), delimiter='\t', names=['Y', 'X'])
...
#     with h5py.File(test_img_path.replace('.jpg','.h5').replace('images','ground_truth'), 'w') as hf:
#         hf['density'] = density
# for validation_img_path in validation_img_paths:
#     ground_truth = pd.read_csv(validation_img_path.replace('.jpg', '.txt'), delimiter='\t', names=['Y', 'X'])
#     img = plt.imread(validation_img_path)
#     img_x = img.shape[0]
#     img_y = img.shape[1]
#     k = np.zeros((img_x, img_y))
...
#     density = gaussian_filter_density(k)
#     with h5py.File(validation_img_path.replace('.jpg','.h5').replace('images','ground_truth'), 'w') as hf:
#         hf['density'] = density